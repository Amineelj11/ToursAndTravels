...
const UpdateTypeForm = () => {
  ...
  const tourTypeProg = type.state;
  const admin_jwtToken = sessionStorage.getItem("admin-jwtToken");

  const [id, setId] = useState(tourTypeProg.id);
  const [name, setName] = useState(tourTypeProg.name);
  const [description, setDescription] = useState(tourTypeProg.description);
  ...
  const saveTransport = (e) => {
    ...
    fetch("http://localhost:8085/api/type/update", {
      method: "PUT",
      headers: {
